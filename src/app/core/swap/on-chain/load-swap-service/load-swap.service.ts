...
import { SwapMode, SwapStatus, LoadedSwap } from '@app/wallet/swap/models/models';
...
import { fromSolidityDecimalString, toBigNumberString } from '@app/shared/helpers/number-utils';
...
export class LoadSwapService {
    ...
    private async ensureAllowance(tokenContractAddress: string, spender: string, amount: number) {
        const allowance = await this.erc20TokenService.allowance(tokenContractAddress, this.authService.getAddress(), spender);
    ...
    private async mapToLoadedSwap(swapId: string, swap: any, mode: SwapMode): Promise<LoadedSwap> {
        if (mode === 'aero_to_erc20') {
